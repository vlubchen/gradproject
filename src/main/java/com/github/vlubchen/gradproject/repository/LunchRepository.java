...
public interface LunchRepository extends BaseRepository<Lunch> {
    ...
    @EntityGraph(attributePaths = {"dish"})
    @Query("""
            SELECT l FROM Lunch l WHERE l.createdDate = :createdDate and l.restaurant.id = :restaurantId
            ORDER BY l.restaurant.id, l.dish.id
            """)
    List<Lunch> getAllByDateAndRestaurantId(LocalDate createdDate, int restaurantId);
